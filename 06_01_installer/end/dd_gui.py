...
class DailyDigestGUI:
    ...
    def __init__(self, root):
        ...
        title_label = ttk.Label(self.__root, text = ' \U0001F4DC Daily Digest \U0001F4DC',
                                font = 'Serif 30 bold', justify = CENTER)
        title_label.pack(padx = 5, pady = 5)

        self.__style = ttk.Style()
        self.__style.configure('TButton', font = ('Gill Sans', 12, 'bold'))
        self.__style.configure('Header.TLabel', font = ('Gill Sans', 20, 'bold')) 
        ...
        self.__events_var = IntVar()
        self.__instagram_var = IntVar()
        self.__article_var = IntVar()
        self.__build_gui_contents(contents_frame,
                                 self.__quote_var, 
                                 self.__events_var,
                                 self.__instagram_var, 
                                 self.__article_var)
        ...
        self.__minute_var.set('45')
        
        self.__quote_var.set(self.__email.content['quote']['include'])
        self.__events_var.set(self.__email.content['events']['include'])
        self.__instagram_var.set(self.__email.content['instagram']['include'])
        self.__article_var.set(self.__email.content['article']['include']) 
    ...
    def __build_gui_recipients(self, master, add_recipient_var, recipient_list_var):
        ...
        recipients_entry = ttk.Entry(master, width = 35, textvariable = add_recipient_var)      
        ...
        recipients_listbox = Listbox(master, listvariable = recipient_list_var,
                                     selectmode  = 'multiple', width = 35, height = 5)
    ...
    def __build_gui_schedule(self, master, hour_var, minute_var):
        ...
        hour_spinbox = ttk.Spinbox(master, from_ = 0, to = 23, textvariable = hour_var,
                                     wrap = True, width = 4, justify = CENTER, font = 'Gill Sans 12')
    ...
    """
    def __build_gui_contents(self, master, quote_var, events_var, instagram_var, article_var):
        # create GUI widgets
        header = ttk.Label(master, text = 'Digest Contents:', style = 'Header.TLabel')
        quote_checkbox = Checkbutton(master, text = 'Be Inspired',
                                     onvalue = True, offvalue = False, 
                                     variable = quote_var)
        weather_checkbox = Checkbutton(master, text = 'Sales Events',
                                          onvalue = True, offvalue = False, 
                                          variable = events_var)
        twitter_checkbox = Checkbutton(master, text = 'Insta Trends',
                                           onvalue = True, offvalue = False,
                                           variable = instagram_var)
        wikipedia_checkbox = Checkbutton(master, text = 'Fashion Article',
                                            onvalue = True, offvalue = False, 
    ...
    def __build_gui_sender(self, master, sender_email_var, sender_password_var):
        ...
        email_entry = ttk.Entry(master, width = 35,
                                textvariable = sender_email_var)
        password_label = ttk.Label(master, text = 'Password:')
        password_entry = ttk.Entry(master, width = 35, show = '*',
    ...
    def __update_settings(self):
        ...
        self.__email.content['events']['include'] = self.__events_var.get()
        self.__email.content['instagram']['include'] = self.__instagram_var.get()
        self.__email.content['article']['include'] = self.__article_var.get()
    ...
    def __manual_send(self):
        ...
        print('Sending email digest manually...')
        self.__email.send_email()
    ...
    def __save_config(self, file_path='dd_config.json'):
        config = {'add_recipient': self.__add_recipient_var.get(),
                  ...
                  'events': self.__events_var.get(),
                  'instagram': self.__instagram_var.get(),
                  'article': self.__article_var.get(),
    ...
    def __load_config(self, file_path='dd_config.json'):
        with open(file_path) as file:
            ...
            self.__events_var.set(config['events'])
            self.__instagram_var.set(config['twitter'])
            self.__article_var.set(config['article'])
